...
impl NatTraversal {
    ...
    async fn query_stun_server(&self, socket: &UdpSocket, stun_server: &str) -> Result<(String, u16), String> {
        // Send STUN binding request
        let stun_request = create_stun_binding_request();
        
        socket.send_to(&stun_request, stun_server)
            .await
...
pub async fn attempt_direct_connection(
    ...
    socket.send_to(hello, peer_addr)
        .await
        .map_err(|e| format!("Send failed: {}", e))?;
...
pub async fn check_peer_reachability(
