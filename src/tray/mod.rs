    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            if connected != last_connected {
                if connected {
                    tray.set_icon(Some(icon_green.clone()));
                    tray.set_tooltip(Some("Mini LAN Bridge — Connected"));
                } else {
                    tray.set_icon(Some(icon_red.clone()));
                    tray.set_tooltip(Some("Mini LAN Bridge — Disconnected"));
                }
