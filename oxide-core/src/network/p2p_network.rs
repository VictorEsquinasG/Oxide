...
use crate::room::{Peer, PeerStatus, Room};
...
impl P2PNetwork {
    ...
    pub async fn new(my_peer_id: String, room: Room) -> Result<Self, String> {
    ...
    pub async fn send_to_peer(&self, peer_id: &str, data: &[u8]) -> Result<(), String> {
        ...
        let conn = conns.get(peer_id).ok_or("Peer not connected")?;
    ...
    pub async fn is_healthy(&self) -> bool {
        ...
        conns
            .iter()
            .any(|(_, conn)| conn.peer.status == PeerStatus::Online)
    }
...
impl P2PPacket {
    pub fn to_bytes(&self) -> Vec<u8> {
        match self {
            P2PPacket::Hello { peer_id } => format!("HELLO:{}", peer_id).into_bytes(),
            P2PPacket::Ping { timestamp } => format!("PING:{}", timestamp).into_bytes(),
            P2PPacket::Pong { timestamp } => format!("PONG:{}", timestamp).into_bytes(),
            P2PPacket::Data { data } => {
            ...
            P2PPacket::Disconnect { reason } => format!("DISC:{}", reason).into_bytes(),
        }
    }

    pub fn from_bytes(data: &[u8]) -> Result<Self, String> {
        let s = String::from_utf8(data.to_vec()).map_err(|e| format!("Invalid UTF-8: {}", e))?;
        ...
        if s.starts_with("HELLO:") {
            ...
            let timestamp = ts_str.parse().map_err(|_| "Invalid timestamp")?;
