...
impl NetworkNode {
    ...
    pub async fn new(
        ...
        state.log(format!(
            "📡 Direct UDP tunnel configured (no TUN device needed)"
        ));
    ...
    pub async fn run(&self, shutdown: Arc<AtomicBool>) -> anyhow::Result<()> {
        ...
        match self.socket.send_to(&hello.encode(), peer_addr).await {
            Ok(bytes) => {
                self.state
                    .log(format!("✅ HELLO sent ({} bytes to {})", bytes, peer_addr));
            }
            Err(e) => {
        ...
        loop {
            ...
            match self.socket.try_recv_from(&mut buf_udp) {
                Ok((n, src)) => {
                    ...
                    self.state
                        .log(format!("📨 Received {} bytes from {}", n, src));
                    ...
                    let is_peer_candidate =
                        !self.state.connected.load(Ordering::Relaxed) && is_same_port;
                    ...
                    if is_expected_peer || is_peer_candidate {
                        ...
                        self.state.log(format!("✅ Packet from peer: {}", src));
                        ...
                        if let Ok(packet) = Packet::decode(&buf_udp[..n]) {
                            match packet.payload {
                                PacketPayload::Control(msg) => match msg {
                                    ...
                                    ControlMessage::HelloAck => {
                                        if !self.state.connected.load(Ordering::Relaxed) {
                                            ...
                                            self.state.update_last_seen();
                                            self.state
                                                .log("✅ Connected (received HELLO_ACK)".into());
                                        } else {
                                            self.state
                                                .log("ℹ️ Additional HELLO_ACK received".into());
                                        }
                                    }
                                    ControlMessage::Ping => {
                                        ...
                                        match self
                                            .socket
                                            .send_to(&pong.encode(), current_peer)
                                            .await
                                        {
                                            Ok(bytes) => {
                                                self.state
                                                    .log(format!("🏓 PONG sent ({} bytes)", bytes));
                                            }
                                            Err(e) => {
                                                self.state
                                ...
                                PacketPayload::Data(_frame) => {
                                    ...
                                    self.state
                                        .log("📦 Data packet received (UDP tunnel)".into());
                                }
                            }
                        } else {
                            self.state
                                .log("⚠️ Failed to decode packet from peer".into());
                        }
                    } else {
