...
impl MeshController {
    ...
    pub async fn initialize_mesh(&self) -> Result<(), String> {
        ...
        self.state
            .log(format!("📡 Initializing P2P mesh for room: {}", room.name));
        self.state.log(format!("🎯 Room ID: {}", room.id));
        self.state
            .log(format!("👥 Players in room: {}", room.peers.len()));
        ...
        if room.peers.len() == 1 {
            ...
            self.state
                .log("✅ Mesh initialized (waiting for other players)".into());
            return Ok(());
        }
        ...
        for (peer_id, peer) in &room.peers {
            let my_marker = if peer_id == &self.state.player_id {
                " ← YOU"
            } else {
                ""
            };
        ...
        let p2p_network = match P2PNetwork::new(my_peer_id, room.clone()).await {
            ...
            Err(e) => {
                self.state
                    .log(format!("❌ Failed to create P2P network: {}", e));
        ...
        {
            let mut p2p_opt = self
                .state
                .p2p_network
                .lock()
                .map_err(|e| format!("Failed to lock p2p_network: {}", e))?;
            *p2p_opt = Some(Arc::new(p2p_network));
        }
    ...
    async fn start_mesh_connections(&self) -> Result<(), String> {
        ...
        match network_arc.start_mesh().await {
            Ok(_) => {
                self.state
                    .log("🔗 P2P mesh started - connecting to peers...".into());
    ...
    pub async fn get_mesh_status(&self) -> MeshStatus {
        let is_active = {
            ...
            p2p_opt
                .and_then(|r| {
                    r.as_ref()
                        .map(|n| n.active.load(std::sync::atomic::Ordering::Relaxed))
                })
                .unwrap_or(false)
        };
    ...
    pub async fn get_peer_metrics(&self, peer_id: &str) -> Option<PeerMetrics> {
        ...
        match network.get_peer_stats(peer_id).await {
            Ok((packets_sent, packets_received, latency_ms)) => Some(PeerMetrics {
