...
use crate::room::{generate_room_code, Peer, PeerStatus, Room, RoomConfig};
...
impl RoomManager {
    ...
    fn get_config_dir() -> Result<PathBuf, String> {
        ...
        #[cfg(target_os = "macos")]
        {
            let home = std::env::var("HOME").map_err(|_| "Failed to get HOME".to_string())?;
    ...
    pub async fn join_room(
        ...
        let room = self.config.get_room_mut(room_id).ok_or("Room not found")?;

        if room.peers.contains_key(&peer_id) {
            let existing_peer = room.get_peer_mut(&peer_id).unwrap();
            existing_peer.alias = peer_alias;
            existing_peer.real_ip = peer_real_ip;
            existing_peer.port = peer_port;
            existing_peer.status = PeerStatus::Online;
            existing_peer.last_seen = std::time::SystemTime::now()
